...
We evaluate four SLMs under two quantization levels across three agentic environments, yielding a $4 \times 2 \times 3 = 24$ configuration grid. Each configuration is tested on 20 tasks, producing 480 planned trials (476 completed; 4 Llama-3.1 tasks terminated due to VRAM pressure). Experiments were conducted March 3--12, 2026, on Google Colab free-tier NVIDIA T4 GPUs ($\sim$15\,GB VRAM). The 60-task benchmark was designed in February 2026, prior to the release of Qwen-3.5-4B (March 2026); task prompts were not optimized for any specific model.
...
Mann-Whitney $U$ tests ($n=20$ per group) reveal that quantization effects are \emph{not uniform}. We distinguish \textbf{robust findings} (non-overlapping CIs, $p<0.05$ with medium+ effect sizes) from \textbf{suggestive trends} ($0.05 < p < 0.10$). \emph{Robust:} In Bash, 4-bit Gemma-3 \emph{significantly outperforms} 8-bit ($\Delta\text{SDM}=+0.200$, $U=255$, $p=0.044$, rank-biserial $r=0.275$). In Python, 8-bit \emph{significantly outperforms} 4-bit ($\Delta\text{SDM}=+0.390$, $U=110.5$, $p=0.009$, $r=0.448$, a medium-to-large effect). \emph{Suggestive:} Qwen-3.5 shows a trending Python advantage for 8-bit ($p=0.078$, $r=0.255$), which should be interpreted cautiously pending replication. All other 9 comparisons are non-significant ($p>0.10$).

\subsection{Qualitative Failure Archetypes}
\label{sec:qualitative}

Through systematic qualitative coding of 476 traces (Section~\ref{sec:coding}), we identify four archetypes (counts are approximate due to single-coder design; see Section~\ref{sec:limitations}):
...
Correct reasoning traces yield truncated code. On the Levenshtein task (py\_8), Gemma-3 8-bit correctly described the DP approach across 5 turns but truncated at \texttt{if s1[i - 1} three times (SyntaxError). The 8-bit variant has significantly fewer truncations ($p=0.009$), linking truncation to 4-bit's degraded long-range syntactic attention. We note that the \texttt{max\_new\_tokens}{$=$}400 budget may also contribute to truncation on longer scripts (10--50 lines); however, the significant quantization-level difference suggests that token budget alone does not explain the effect. The Actor-Critic provides within-turn repair but cannot prevent cross-turn recurrence.
...
Across 335 failures: Format/Parser Errors dominate (42.7\%), followed by Max Turns Exhausted (31.6\%), Immediate Failures (9.3\%), Judge Rejected (9.3\%), and Self-Correction Loops (7.2\%). Error distributions vary systematically by model (Fig.~\ref{fig:errors}). Web Synthesis is the most forgiving environment ($\overline{\text{SDM}}=0.273$); Bash the most challenging ($\overline{\text{SDM}}=0.082$); Python occupies the middle ($\overline{\text{SDM}}=0.232$) where iterative debugging partially compensates for initial errors (Fig.~\ref{fig:env_perf}). Note that Web retains the full trajectory context while Bash and Python use a 2-turn sliding window (Section~\ref{sec:limitations}), which may partially contribute to Web's higher SDM independent of task difficulty. Seven tasks achieved 0\% success across all configurations, all requiring $\geq$4 interdependent steps.
