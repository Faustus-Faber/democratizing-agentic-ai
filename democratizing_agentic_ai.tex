...
ReAct~\cite{yao2022react} established thought-action-observation loops; Toolformer~\cite{schick2023toolformer} taught models API use via self-supervision; Reflexion~\cite{shinn2023reflexion} introduced verbal reinforcement learning related to our Actor-Critic loop; SWE-agent~\cite{yang2024sweagent} applied agentic loops to software engineering. Surveys~\cite{wang2024survey,xi2023rise} establish that effective agency requires compositional capabilities (planning, tool use, memory, self-correction), building on chain-of-thought reasoning~\cite{wei2022chainofthought}. Among benchmarks, AgentBench~\cite{liu2023agentbench} tests across 8 environments, WebArena~\cite{zhou2024webarena} and OSWorld~\cite{xie2024osworld} target realistic settings, T-Eval~\cite{chen2024teval} evaluates step-by-step tool use, and ToolLLM~\cite{qin2024toolllm} scales to 16K+ APIs. ACBench~\cite{dong2025acbench} (ICML 2025) most directly evaluates \emph{compressed} agents but tests only single-turn tool calls. Our work fills this gap with multi-turn recursive evaluation.

\paragraph{Quantization and Reasoning.}
COLM 2025~\cite{colm2025quantization} shows quantization disproportionately impacts reasoning, but focuses on static benchmarks. Huang and Chang~\cite{huang2023reasoning} establish reasoning as a multi-component capability; SLM surveys~\cite{slm2025survey} identify the need for deployment recommendations. However, no prior work examines how quantization affects \emph{compound, multi-turn} agentic reasoning where errors cascade across turns $t \rightarrow t{+}k$.
...
Gemma-3 dominates ($\overline{\text{SDM}}=0.403$, 53.3\% success). Phi-4-Mini (30.0\%) and Qwen-3.5 (33.3\%) perform comparably, while Llama-3.1 exhibits near-total failure (0.86\%, 1/116 tasks), despite being the largest model at 8B.

\subsection{4-bit vs.\ 8-bit: Environment-Dependent Effects}

Mann-Whitney $U$ tests ($n=20$ per group) reveal that quantization effects are \emph{not uniform}. In Bash, 4-bit Gemma-3 \emph{significantly outperforms} 8-bit ($\Delta\text{SDM}=+0.200$, $U=255$, $p=0.044$). In Python, 8-bit \emph{significantly outperforms} 4-bit ($\Delta\text{SDM}=+0.390$, $U=110.5$, $p=0.009$). Qwen-3.5 shows a trending Python advantage for 8-bit ($p=0.078$). All other 9 comparisons are non-significant.
...
Our qualitative analysis reveals that agentic reasoning is not monolithic but comprises four independently degradable sub-capabilities: (i)~\emph{plan decomposition}; (ii)~\emph{state tracking}; (iii)~\emph{format adherence}; (iv)~\emph{completion signaling}~\cite{wang2024survey,xi2023rise}. Qwen-3.5 excels at (i) but fails at (iii); Llama-3.1 fails entirely at (ii); Gemma-3 maintains all four consistently; Phi-4-Mini fails at (iv). Table~\ref{tab:arch_failure} maps each model's dominant failure to its architectural root cause and a targeted, deployable mitigation.
...
Llama-3.1 & State Tracking & TOON compression loses early context & Explicit state summaries \\
...
\begin{enumerate}[nosep,leftmargin=*]
  \item \textbf{Single-run design.} Each configuration was run once ($n{=}20$ tasks) without repeated seeds. We mitigate this via non-parametric Mann-Whitney~$U$ tests (which make no distributional assumptions) and report exact $p$-values; however, bootstrap confidence intervals and multi-seed replication would further strengthen effect-size estimates. The large, consistent effect sizes observed (e.g., Llama-3.1 at 0.86\% vs.\ Gemma-3 at 53.3\%) suggest robustness, but marginal comparisons (e.g., Qwen Python $p{=}0.078$) should be interpreted cautiously.
...
We release our evaluation harness, all 476 reasoning traces, and the combined dataset to support future research in quantized agentic AI on edge hardware.

\begin{ack}
We thank the open-source teams behind Qwen, Llama, Gemma, and Phi, the \texttt{bitsandbytes} and HuggingFace communities, and Google Colab for free GPU access. \textbf{Reproducibility:} All experiments used Colab free-tier T4 GPUs, March 3--12, 2026. The harness, 24 trace files, and dataset (476$\times$16) are at the project repository (redacted).
\end{ack}
